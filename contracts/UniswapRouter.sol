...
contract UniswapRouter is IUniswapRouter {
    ...
    function _addLiquidity(
        ...
        if (IUniswapFactory(_factory).getPair(tokenA, tokenB) == address(0)) {
            IUniswapFactory(_factory).createPair(tokenA, tokenB);
        }
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        (uint256 amount0, uint256 amount1) = IUniswapPair(pair).burn(to);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ...
                : _to;
            IUniswapPair(UniswapV2Library.pairFor(_factory, input, output)).swap(
                amount0Out,
                amount1Out,
                to,
                new bytes(0)
